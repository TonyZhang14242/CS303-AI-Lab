...
class ProblemSolvingAgent:
    ...
    def DFS(self, obstacles, start_pos, goal_pos):
        ...
        while frontier:
            ...
            if node == goal_pos:
                ...
                path.reverse()
                return path, visited
            for neighbor in self.neighbours_of(obstacles, node):
                neighborpos = neighbor[0]
                if neighborpos not in list(set(frontier) | set(visited)):
                    prev[neighborpos] = node
                    frontier.append(neighborpos)
    ...
    def UCS(self, obstacles, start_pos, goal_pos):
        ...
        while frontier:
            ...
            if node[1] == goal_pos:
                ...
                path.reverse()
                return path, visited
            for neighbor in self.neighbours_of(obstacles, node[1]):
                neighborpos = neighbor[0]
                ...
                for i in frontier:
                    if i[1] == neighborpos:
                        is_in_frontier = True
                if neighborpos not in visited and is_in_frontier == False:
                    heapq.heappush(frontier, (neighborcost + node[0], neighborpos))
                    postonode[neighborpos] = neighborcost + node[0]
                    prev[neighborpos] = node[1]
                elif is_in_frontier:
                    if node[0] + neighborcost < postonode[neighborpos]:
                        frontier.remove((postonode[neighborpos], neighborpos))
                        heapq.heapify(frontier)
                        heapq.heappush(frontier, (node[0] + neighborcost, neighborpos))
                        postonode[neighborpos] = node[0] + neighborcost
                        prev[neighborpos] = node[1]
            ...
            for neighbor in self.neighbours_of(obstacles, node[1]):
                ...
                for i in frontier:
                    if i[1] == neighbor[0]:
                        is_in_frontier = True
                if neighborpos not in visited and is_in_frontier == False:
                    hdis=self.euclidean_distance(neighborpos, goal_pos)
                    heapq.heappush(frontier,(hdis, neighborpos))
                    prev[neighborpos] = node[1]
                elif is_in_frontier:
                    pass
    ...
    def Astar(self, obstacles, start_pos, goal_pos):
        ...
        g = {}
        g[start_pos] = 0
        frontier.append((self.euclidean_distance(start_pos,goal_pos), start_pos))
        heapq.heapify(frontier)
        while frontier:
            node = heapq.heappop(frontier)
            #for v in frontier:
             #   if node==None or g[v]+self.euclidean_distance(v,goal_pos)< g[node]+self.euclidean_distance(node,goal_pos):
              #      node=v
            visited.append(node[1])
            ...
            for neighbor in self.neighbours_of(obstacles, node[1]):
                ...
                if neighborpos not in visited and is_in_frontier == False:
                    prev[neighborpos]= node[1]
                    g[neighborpos]= g[node[1]] + neighborcost
                    heapq.heappush(frontier, (g[neighborpos]+self.euclidean_distance(neighborpos,goal_pos),neighborpos))
                elif is_in_frontier:
                    if g[node[1]] + neighborcost < g[neighborpos]:
                        frontier.remove((g[neighborpos] + self.euclidean_distance(neighborpos, goal_pos), neighborpos))
                        g[neighborpos] = g[node[1]] + neighborcost
                        heapq.heapify(frontier)
                        heapq.heappush(frontier, (g[neighborpos]+self.euclidean_distance(neighborpos, goal_pos), neighborpos))
                        prev[neighborpos] = node[1]
